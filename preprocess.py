...
def read_pdf_from_url(url):
 ...
 pdf= pdfplumber.open(io.BytesIO(reponse.content))
 text= ""
...
def preprocess(data):
  for i in range(len(data)):
      if len(data)>= 2 and data[i][:2].isdigit(): data[i]= data[i][2:]
  return data
...
def crop_judgment(judgment):
    ...
    jfull_raw= list(filter(None, preprocess(read_pdf_from_url(judgment["JPDF"]).splitlines())))
